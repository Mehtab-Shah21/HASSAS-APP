...
export default function InvoiceDetailPage() {
  ...
  if (loading || !invoice) return <p className="text-sm text-slate-500">Loading...</p>;

  const balanceDue = invoice.grand_total - invoice.amount_paid;

  return (
    <div>
      <button onClick={() => navigate("/invoices")} className="mb-3 text-sm text-slate-500 hover:underline">
        ← Back to invoices
      </button>
            ...
            <table className="w-full text-sm">
              <tbody className="divide-y divide-slate-100">
                {invoice.payments.map((p) => (
                  <tr key={p.id}>
                    <td className="py-1.5 text-slate-600">{p.paid_on}</td>
                    <td className="py-1.5 capitalize text-slate-600">{p.method}</td>
                    <td className="py-1.5 text-slate-600">{p.reference}</td>
                    <td className="py-1.5 text-right text-slate-700">{p.amount.toFixed(2)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
